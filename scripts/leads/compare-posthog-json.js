...
// Configuration
const JSON_FILE = 'apps/sovoli.com/src/modules/data/organisations/vocational-school/jamaica/healingemeraldwellness/leads.json';
...
if (fs.existsSync(POSTHOG_EVENTS_FILE)) {
  try {
    ...
    console.log(`✅ Loaded ${posthogEvents.length} events from ${POSTHOG_EVENTS_FILE}\n`);
  } catch (e) {
  ...
  console.error('   Run extract-posthog-events.js first to generate this file.');
  process.exit(1);
}
...
posthogEvents.forEach(phEvent => {
  ...
  const phLoggedAt = new Date(phEvent.loggedAt).toISOString().replace(/\.\d{3}Z$/, 'Z');
  ...
  if (lead && lead.interactions) {
    for (const jsonInteraction of lead.interactions) {
      const jsonLoggedAt = new Date(jsonInteraction.loggedAt).toISOString().replace(/\.\d{3}Z$/, 'Z');
      ...
      if (timeDiff < 5000) {
        ...
        if (phEvent.contactOutcome !== jsonInteraction.contactOutcome) {
          differences.push(`contactOutcome: PostHog="${phEvent.contactOutcome}", JSON="${jsonInteraction.contactOutcome}"`);
        }
        if (phEvent.notReachedReason !== jsonInteraction.notReachedReason) {
          differences.push(`notReachedReason: PostHog="${phEvent.notReachedReason}", JSON="${jsonInteraction.notReachedReason}"`);
        }
        if (phEvent.interestLevel !== jsonInteraction.interestLevel) {
          differences.push(`interestLevel: PostHog="${phEvent.interestLevel}", JSON="${jsonInteraction.interestLevel}"`);
        }
        if (phEvent.blocker !== jsonInteraction.blocker) {
          differences.push(`blocker: PostHog="${phEvent.blocker}", JSON="${jsonInteraction.blocker}"`);
        }
        if (phEvent.nextAction !== jsonInteraction.nextAction) {
          differences.push(`nextAction: PostHog="${phEvent.nextAction}", JSON="${jsonInteraction.nextAction}"`);
        }
        if (phEvent.notes !== jsonInteraction.notes) {
          differences.push(`notes: PostHog="${phEvent.notes}", JSON="${jsonInteraction.notes}"`);
        }
...
console.log(`❌ Missing Interactions (in PostHog but not in JSON): ${missingInteractions.length}`);
...
if (missingInteractions.length > 0) {
  ...
  missingInteractions.forEach(({ phEvent, lead }) => {
    ...
    if (phEvent.interestLevel) console.log(`  Interest Level: ${phEvent.interestLevel}`);
...
const fieldsInPostHogNotInJSON = [...posthogFields].filter(f => !jsonFields.has(f) && f !== 'timestamp' && f !== 'tenant');
const fieldsInJSONNotInPostHog = [...jsonFields].filter(f => !posthogFields.has(f));
...
posthogEvents.forEach(phEvent => {
  const lead = jsonData.leads.find(l => l.id === phEvent.leadId);
  if (lead) {
    console.log(`✅ Lead ${phEvent.leadId}: Found in JSON (${lead.name}, ${lead.phone})`);
  } else {
    console.log(`❌ Lead ${phEvent.leadId}: NOT FOUND in JSON - NEW LEAD DETECTED`);
  }
});
...
if (recommendations.length === 0) {
  ...
  recommendations.forEach((rec, i) => {
    console.log(`${i + 1}. [${rec.priority}] ${rec.issue} (${rec.count} occurrence(s))`);
