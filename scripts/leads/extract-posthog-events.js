...
if (!inputFile || !fs.existsSync(inputFile)) {
  ...
  console.error('Usage: node scripts/leads/extract-posthog-events.js <posthog-results-file>');
  process.exit(1);
}
...
function findLatestPostHogFile() {
  ...
  const agentToolsPath = path.join(process.env.HOME || process.env.USERPROFILE, '.cursor', 'projects', 'e-Developers-sovoli', 'agent-tools');
  if (!fs.existsSync(agentToolsPath)) {
    return null;
  }
  
  const files = fs.readdirSync(agentToolsPath)
