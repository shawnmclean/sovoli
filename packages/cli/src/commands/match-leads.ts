...
function displayResults(results: ExtractionResult[]): void {
  ...
  // Summary
  const withMatchesCount = results.filter(
    (r) => r.matchedOrgs.length > 0,
  ).length;
...
export const matchLeadsCommand = new Command()
  ...
  .action(async () => {
    ...
    for (const filePath of extractionFiles) {
      ...
      if (!businessName) {
        console.warn(`No business name found in ${path.basename(filePath)}`);
        continue;
      }
      ...
      if (orgMatches.length > 0) {
        ...
        if (orgDir) {
          ...
          for (let i = 0; i < extraction.extraction.programs.length; i++) {
