  ...
  .action(
    async (options: {
      ...
      try {
        ...
        console.log(
          `✅ Found ${phoneEnteredEvents.length} LeadPhoneEntered event(s)\n`,
        );
        ...
        console.log(
          `✅ Found ${nameEnteredEvents.length} LeadNameEntered event(s)\n`,
        );
        ...
        for (const event of leadEvents) {
          ...
          if (!lead) {
            ...
            if (props.type && !lead.type)
              lead.type = props.type as string | undefined;
            if (props.content_category && !lead.content_category)
              lead.content_category = props.content_category as
                | string
                | undefined;
            if (props.content_name && !lead.content_name)
              lead.content_name = props.content_name as string | undefined;
            if (props.content_type && !lead.content_type)
              lead.content_type = props.content_type as string | undefined;
            if (props.content_ids && !lead.content_ids)
              lead.content_ids = props.content_ids as string[] | undefined;
            if (props.value && !lead.value)
              lead.value = props.value as number | undefined;
            if (props.currency && !lead.currency)
              lead.currency = props.currency as string | undefined;
            if (props.programName && !lead.programName)
              lead.programName = props.programName as string | undefined;
            if (props.cycleLabel && !lead.cycleLabel)
              lead.cycleLabel = props.cycleLabel as string | undefined;
        ...
        if (newLeadsCount > 0 || updatedLeadsCount > 0) {
          console.log(
            `🆕 Created ${newLeadsCount} new lead(s), updated ${updatedLeadsCount} lead(s)\n`,
          );
        }
        ...
        for (const event of updatedEvents) {
          ...
          const loggedAt =
            (event.properties.loggedAt as string) || event.timestamp;
          ...
          if (!interactionExists) {
            leadToUpdate.interactions = [...existingInteractions, interaction];
        ...
        if (skippedLeads > 0) {
          console.log(
            `⚠️  Skipped ${skippedLeads} interaction(s) for leads not found in JSON\n`,
          );
        }
        ...
        for (let i = 0; i < updatedLeads.length; i++) {
          ...
          if (lead.id.startsWith("lead-") && lead.phone) {
            ...
            if (distinctId) {
              ...
              const existingLeadIndex = updatedLeads.findIndex(
                (l) => l && l.id === distinctId,
              );
        ...
        if (migratedCount > 0 || leadsToRemove.length > 0) {
          console.log(
            `✅ Migrated ${migratedCount} lead(s) from old IDs to distinct_id format`,
          );
          if (leadsToRemove.length > 0) {
            console.log(
              `   Removed ${leadsToRemove.length} duplicate lead(s)\n`,
            );
          } else {
            console.log();
          }
        ...
        console.log(
          `✅ Processed ${phoneEnteredEvents.length} LeadPhoneEntered event(s)`,
        );
        console.log(
          `✅ Processed ${nameEnteredEvents.length} LeadNameEntered event(s)`,
        );
        console.log(`✅ Processed ${leadEvents.length} Lead event(s)`);
        console.log(
          `✅ Processed ${updatedEvents.length} LeadUpdated event(s)`,
        );
        ...
        if (hasChanges) {
          ...
          console.log(
            "✅ All leads and interactions are already synced. No updates needed.\n",
          );
        }
      } catch (error) {
