...
import {
  trackChanges,
  transformExtractionToOrgUpdates,
  transformExtractionToProgramUpdates,
} from "../utils/extraction-transform.js";
import {
  applyOrgChanges,
  createNewOrg,
  loadOrgFile,
  saveOrgFile,
} from "../utils/org-file-ops.js";
...
import {
  ...
  loadProgramFile,
  saveProgramFile,
} from "../utils/program-file-ops.js";
import {
...
async function processProgram(
  ...
  const action = await promptProgramAction(
    programEvidence.name,
    matchedPrograms,
  );
  ...
  if (action === "update" && matchedPrograms && matchedPrograms.length > 0) {
    ...
    // Merge updates
    const updatedProgram = applyProgramChanges(existingProgram, updates, {
      source: extractionFilename,
      timestamp: new Date().toISOString(),
      updatedFields: Object.keys(updates),
      isNew: false,
  ...
  if (pendingOrgChanges.length > 0) {
    ...
    for (const change of pendingOrgChanges) {
      if (change.type === "update") {
        ...
        console.log(
          `     Updated fields: ${change.changes.updatedFields.join(", ") || "none"}`,
        );
      } else {
