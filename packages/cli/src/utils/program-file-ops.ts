...
export function loadProgramFile(
  ...
  const program = result.programs.find((p) => (p.id as string) === programId);
...
export function saveProgramFile(
  ...
  if (changes.isNew) {
    (programWithMetadata as { _addedAt?: string })._addedAt = changes.timestamp;
