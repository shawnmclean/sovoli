    ...
    for (const program of programsData) {
      ...
      if (programInfo.action === "add") {
        ...
        programsToUpdate.push({
          ...
          name: program.programName,
          targetId:
            targetProgramId !== program.programId ? targetProgramId : undefined,
        });
      }
    }
        ...
        if (result.orgDir) {
          for (const program of programsData) {
            ...
            if (programInfo?.data && programInfo.action === "add") {
              ...
              const editedSchedule = programInfo.data._extractedSchedule as {
                dates?: string[];
              } | null;
              const editedPricing = programInfo.data._extractedPricing as
                | ProgramEvidence["pricing"]
                | null;
              ...
              const programResult = await createNewProgram(
                ...
                (editedPricing ??
                  program.pricing) as ProgramEvidence["pricing"],
              );
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin/imports">
            <Button variant="light">Back to Imports</Button>
          </Link>
        </div>
        ...
        <div className="space-y-4">
          ...
          <ProgramConfiguration
            ...
            onApplyToAllPrograms={(suffix) => {
              ...
              for (const program of programsData) {
                const currentData =
                  editedPrograms[program.programId]?.data ??
                  program.transformedData;
                const currentName =
                  (currentData.name as string) || program.programName;
        ...
        <div className="flex justify-end gap-4 pt-4 border-t">
          <Link href="/admin/imports">
            <Button variant="light" disabled={isCommitting}>
              Cancel
            </Button>
          </Link>
          <Button
